...
public class ChangingUsersDataTests {
    ...
    @Test
    @DisplayName("Changing a user's password with authorization")
    @Description("API test editing an authorized user's password. " +
            "Expected result - password changed")
    public void changeUserPasswordWithAuthIsSuccessTest() {
        String newPassword = "2" + password;
        Response response = user.updateUser(email, newPassword, name, token);
        check.checkStatusCode(response, SC_OK);
        check.checkSuccessStatus(response, "true");
        user.checkUserData(response, email, newPassword, name);
    }
    ...
    @Test
    @DisplayName("Changing a user's password without authorization")
    @Description("API test editing an unauthorized user's password. " +
            "Expected result - the password has not been changed, an error message has been received")
    public void changeUserPasswordWithoutAuthIsSuccessTest() {
        String newPassword = "2" + password;
        Response response = user.updateUser(email, newPassword, name);
        check.checkStatusCode(response, SC_UNAUTHORIZED);
        check.checkSuccessStatus(response, "false");
        check.checkMessageText(response, "You should be authorised");
    }
